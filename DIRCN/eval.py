        ...
        for i_subject in range(58):
            ...
            for i_slice in range(num_slices):
                ...
                with h5py.File(y_fname, "r") as hf:
                    ...
                    recon = torch.from_numpy(recon).to(device=device).type(torch.float32)
                ssim_ele = ssim_calculator(recon * mask, target * mask, maximum).cpu().numpy()
                # ssim_ele = ssim_calculator(recon, target, maximum).cpu().numpy()
                print(ssim_ele)
                ssim_total += ssim_ele
                idx += 1
...
if __name__ == '__main__':
    ...
    parser.add_argument('-yp', '--your_data_path', type=str, default='../result/DIRCN')
